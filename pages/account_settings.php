<?php
    ...
    if ((!empty($_POST['nom']) && !empty($_POST['old_password']) && !empty($_POST['password']) &&
        ...
        if ($_FILES['pdp']['size'] <= 5000000) { //5Mo max
            // Récupérer les informations du fichier
            $file_info = pathinfo($_FILES['pdp']['name']);
            // On récupère son extension
            $file_extension = $file_info['extension'];
            //Extension(s) acceptée(s)
            $array_extension = array('PNG', 'png', 'JPEG', 'jpeg', 'JPG', 'jpg'); // png, jpeg and jpg only
            // Répertoire par défaut + nom du fichier
            $address = '../src/images/photo_profile/'.$_FILES['pdp']['name'];

            // Si les extensions sont bonnes alors on envoie
            if (in_array($file_extension, $array_extension)) {
                move_uploaded_file($_FILES['pdp']['tmp_name'], $address);
                // Envoie à la bdd
                $req = $bdd -> prepare('INSERT INTO files (name, url) VALUES (?, ?)');
                $req -> execute(array($_FILES['pdp']['name'], $address));
                $req = $bdd->prepare('SELECT url FROM files WHERE name = ?');
                $req->execute(array($_FILES['pdp']['name']));
                $data = $req->fetchColumn();
                $_SESSION['pdp'] = $data;
                header('location: ../pages/account_settings.php?success=1?pdp=1');
                exit();
            } else {
                header('location: ../pages/account_settings.php?error=1&extension=1');
                exit();
            }
        }
    }
...
<head>
    ...
    <meta charset="UTF-8">
    <link rel="stylesheet" type="text/css" href="../src/style/account_settings.css"/>
...
<body>
    <div id="main">
        ...
        <form method="post" action="account_settings.php" enctype="multipart/form-data">
            ...
            <input type="password" name="conf_password" placeholder="Confirmation du mot de passe" required>
